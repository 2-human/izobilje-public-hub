...
(function(){
const CFG = (window.IZOBILJE_REVIEW_CONFIG) || {};
...
/* C17 · library/features/review-widget/commentable-everything.md → anchorEmpty.
   A tag listed in IZOBILJE_REVIEW_CONFIG.anchorEmpty bypasses the direct-text gate
   ...
   ANCHOR_CELLS / window.IZOBILJE_REVIEW_ANCHOR_ALL core-gate flag. */
...
function isInSiteChrome(el){ /* Chrome is normally excluded because identical nav/footer repeats across a multi-page site, so one comment would appear on every page. A single-page hub has no such duplication, and excluding chrome there just makes the sidebar and footer uncommentable. Opt in with anchorChrome. */ if(CFG.anchorChrome) return false; return !!el.closest('nav, header[role="banner"], footer'); }
 ...
 *
 * Contract:
 *   1. The page declares its dimensions in `window.IZOBILJE_VARIANT_DIMENSIONS`,
...
function _dims(){ return window.IZOBILJE_VARIANT_DIMENSIONS || {}; }
...
window.addEventListener('message', e => {
  if(!e || !e.data || e.data.type !== 'izobilje:lp-catalog') return;
...
function _slots(){ return (window.IZOBILJE_SLOTS && Array.isArray(window.IZOBILJE_SLOTS.declared)) ? window.IZOBILJE_SLOTS.declared : []; }
function _broadcastPrefixes(){ return (window.IZOBILJE_SLOTS && Array.isArray(window.IZOBILJE_SLOTS.broadcastDefaultPrefixes)) ? window.IZOBILJE_SLOTS.broadcastDefaultPrefixes : []; }
...
function reviewer(){let n=store.get('izobilje_reviewer');if(!n){let r=null;/* prompt() is unavailable in some embedded/preview contexts and THROWS there. Unguarded, that exception aborts widget init before any anchor is placed, so the whole review mode dies rather than degrading. */try{r=window.prompt(L.namePrompt,'');}catch(e){r=null;}n=String(r||'Anonymous').trim()||'Anonymous';store.set('izobilje_reviewer',n);}return n;}
...
function localAdapter(){
  const KEY='izobilje_review_comments'; let cb=null;
...
/* Embedded-media anchor pass (images since 2026-06-24; iframes added 2026-08-16).
 * Extended from <img> to <iframe> because this hub's flagship artefact IS an iframe (the
 * embedded website), and iframe is in NEVER_ANCHOR, so without this the one view that
 * matters most would be the only uncommentable thing in the hub.
 * Images themselves are in NEVER_ANCHOR
...
function anchorImagePass(){
  document.querySelectorAll('img, iframe').forEach(img=>{
    ...
    let prefix = img.tagName.toLowerCase() === 'iframe' ? 'embed' : 'img';
...
function render(){
   ...
   * on every LP whose IZOBILJE_SLOTS.declared includes the slot AND whose SLUG
...
(async function init(){
  ...
  /* variant-aware anchors (upstream variation): the prototype rewrites a zone's
